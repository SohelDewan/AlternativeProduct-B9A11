...
async function run() {
  try {
    ...
    app.post("/recommendation", async (req, res) => {
      ...
      const updateRecommendationCount = await productCollection.updateOne(
        productQuery,
        updateDoc
      );
    ...
    // get all products data for pagination
    app.get("/queries", async (req, res) => {
      ...
      let query = {};
      if (filter) query = { brandName: filter }
      let options = {}
      // if (sort) options.sort = sort === "asc"? 1 : -1;
      if (sort) options = { sort: {datePosted : sort === "asc" ? 1 : -1 }}
